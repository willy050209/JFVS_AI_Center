...
* **設計邏輯**：  
  ...
  * **延遲優化：模型預熱 (Model Warm-up)**：OpenVINO 在首次執行時需要進行模型編譯 (JIT)，這會導致第一次辨識出現數秒卡頓。本專案實作了預熱機制，在系統啟動後自動傳送 0.5 秒的模擬音訊進行背景推論，確保使用者第一次說話時模型已處於最佳狀態。
  * **強健性設計：延遲載入 (Lazy Loading)**：為了避免在啟動時因模型尚未下載完成而崩潰，推論服務採用了延遲初始化模式，只有在確認檔案存在且真正需要時才會載入推論引擎。
...
* **設計邏輯**：  
  * **環境初始化編排器 (Resource Initialization Orchestration)**：伺服器啟動是一個複雜的過程。我們將 FFmpeg 下載、Whisper 模型準備、Piper 引擎部署等任務抽象化為 `IResourceInitializer`。透過一個統一的 `ModelManagerService` 依照優先級執行，確保所有神經元在服務對外開放前都已正確連結。
